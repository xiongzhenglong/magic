...
$sql="select * from hm_user where daili='$uname' order by id desc";
...
$sql="select * from hm_user where daili='$uname' order by id desc limit $kaishi,$pagesize";//取得记录从计算出的初始值开始，一共$pagesize条
//print_r($sql);
...
if($pages<=5){
	for($i=1;$i<=$pages;$i++){
		$s1.='<li class="paginate_button '.getzt1($page,$i).'"><a href="javascript:void(0);" onclick="gettgyh('.$i.')">'.$i.'</a></li>';
	}
}else{
	if($page<=3){
		for($i=1;$i<=5;$i++){
